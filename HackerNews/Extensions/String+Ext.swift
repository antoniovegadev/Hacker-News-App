...
extension String {
    ...
    func decodedHTML() -> NSMutableAttributedString {
        let htmlDecoded = self.decodingHTMLEntities()
        let paragraphDecoded = htmlDecoded.replacingOccurrences(of: "<p>", with: "\n\n")
        let italicsDecoded = paragraphDecoded.replacingOccurrences(of: #"(<\/{0,1}i>)"#, with: "*", options: .regularExpression)

        let pattern = #"<a href="(.+)"(?: rel=".*")>(.+)<\/a>"#

        let nsrange = NSRange(italicsDecoded.startIndex..<italicsDecoded.endIndex, in: italicsDecoded)

        guard let regex = try? NSRegularExpression(pattern: pattern, options: []) else {
            return NSMutableAttributedString(string: italicsDecoded)
        }

        let result = NSMutableAttributedString(string: "")
        var position = italicsDecoded.startIndex

        regex.enumerateMatches(in: italicsDecoded, options: [], range: nsrange) { match, flags, stop in
            guard let match = match else { return }

            if match.numberOfRanges == 3,
               let matchRange = Range(match.range(at: 0), in: italicsDecoded),
               let urlRange = Range(match.range(at: 1), in: italicsDecoded),
               let textRange = Range(match.range(at: 2), in: italicsDecoded) {
                let rangePreceedingMatch = position..<matchRange.lowerBound
                let str = italicsDecoded[rangePreceedingMatch]
//                result.append(contentsOf: italicsDecoded[rangePreceedingMatch])
                result.append(NSAttributedString(string: String(str)))

                let matchStr = String(italicsDecoded[matchRange])
                let text = String(italicsDecoded[textRange])
                let url = String(italicsDecoded[urlRange])
//                let replacement = "[\(text)](\(url))"

                result.append(NSAttributedString(string: text))
                let resultStr = result.string
                let range: Range<String.Index> = resultStr.range(of: text)!
                let index: Int = resultStr.distance(from: resultStr.startIndex, to: range.lowerBound)

                result.addAttribute(.link, value: url, range: NSRange(location: index, length: text.count))
                print("\(result.string.count) - \(match.range(at: 0).lowerBound) - \(text.count)")

                position = italicsDecoded.index(matchRange.lowerBound, offsetBy: matchStr.count)
            }
        }

        if position != italicsDecoded.endIndex {
            let str = italicsDecoded[position..<italicsDecoded.endIndex]
            result.append(NSAttributedString(string: String(str)))
        }
